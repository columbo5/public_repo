...
echo "$($PSQL "select t1.name from games full join teams t1 on games.winner_id = t1.team_id where year=2014 and round='Eighth-Final' UNION select t2.name from games full join teams t2 on games.opponent_id = t2.team_id where year=2014 and round='Eighth-Final';")"
